...
defineOptions({
  name: "UserRoleGeneralUpdateModal",
});
...
// Selected role items for USelectMenu (stores the actual objects)
...
// Selected roles for display as list
const selectedRolesList = computed(() => {
  if (!selectedRoles.value || selectedRoles.value.length === 0) {
    return [];
  }
  return props.roles.filter((r) =>
    selectedRoles.value.some((sr) => sr.value === r.id),
  );
});

// Extract role IDs for API submission
const selectedRoleIds = computed(() => {
  return selectedRoles.value.map((r) => r.value);
});
...
async function onSubmit() {
  ...
  try {
    await $fetch(
      `http://${ip.ipBackEnd}/api/userRoleBranchUnit/${props.user.nik}`,
      {
        method: "PUT",
        headers: {
