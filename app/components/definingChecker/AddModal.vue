...
// Get all checker NIKs already assigned to any group in this event
const assignedCheckerNikSet = computed(() => {
  ...
  groups.forEach((group) => {
    group.checkerGroups?.forEach((cg) => {
      if (cg.checker) assignedNiks.add(cg.checker);
    });
  });
  return assignedNiks;
});

// Get all member NIKs already assigned to any group in this event
const assignedMemberNikSet = computed(() => {
  const groups = selectedEvent.value?.groups || [];
  const assignedNiks = new Set<string>();
  groups.forEach((group) => {
    group.groupMembers?.forEach((gm) => {
      if (gm.member) assignedNiks.add(gm.member);
    });
  });
  return assignedNiks;
});
