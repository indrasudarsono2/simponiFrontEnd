...
const schema = z.object({
  branchUnitId: z.number().min(1, "Please select a branch unit"),
  selectedUserNiks: z
    .array(z.string())
...
// Computed property for user options (only users without branchUnit)
const userOptions = computed(() => {
  return props.users
    .filter((user) => !user.branchUnit)
    .map((user) => ({
      value: user.nik,
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  ...
  try {
    ...
    toast.add({
      title: "Success",
      description: `Assigned ${event.data.selectedUserNiks.length} user(s) to branch unit`,
      color: "success",
    });
    ...
    <UModal v-model:open="open" @close="handleClose">
      <template #header>
        <div class="flex items-center gap-2">
          ...
          <h3 class="text-lg font-semibold">Assign Users to Branch Unit</h3>
        </div>
      </template>
      ...
      <template #body>
        ...
        >
          ...
          <UFormField label="Branch Unit" name="branchUnitId" required>
            <USelect
              v-model="state.branchUnitId"
              :items="branchUnitOptions"
              placeholder="Select a branch unit"
              class="w-full"
            />
          </UFormField>
