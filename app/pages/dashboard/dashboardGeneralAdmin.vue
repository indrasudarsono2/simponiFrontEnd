...
const { token } = useAuth();
const selectedDutyBranch = ref("all");
...
const { data, status, error, refresh } = await useFetch<DashboardGeneralAdmin>(
  `http://${ip.ipBackEnd}/api/dashboardGeneralAdmin`,
  {
    headers: computed(() => ({
...
const dutyBranchTabs = computed(() => {
  const dutyUsers = data.value?.activeDutyUsers ?? [];
  const tabs = (data.value?.branchDistribution ?? []).map((item) => ({
    label: `${item.branch} (${dutyUsers.filter((user) => user.branch === item.branch).length})`,
    value: item.branch,
  }));

  if (dutyUsers.some((user) => !user.branch)) {
    tabs.push({
      label: `Unassigned (${dutyUsers.filter((user) => !user.branch).length})`,
      value: "__unassigned__",
    });
  }

  return [
    { label: `All Branches (${dutyUsers.length})`, value: "all" },
    ...tabs,
  ];
});

const filteredDutyUsers = computed(() => {
  const users = data.value?.activeDutyUsers ?? [];
  if (selectedDutyBranch.value === "all") return users;
  if (selectedDutyBranch.value === "__unassigned__") {
    return users.filter((user) => !user.branch);
  }
  return users.filter((user) => user.branch === selectedDutyBranch.value);
});
...
<template>
  <UDashboardPanel id="dashboard-general-admin">
    <template #header>
      <UDashboardNavbar title="General Admin Dashboard">
        <template #leading>
            ...
            <template #header>
              <div class="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p class="text-sm text-muted">
                    Operational issues from the last month across all branches.
                  </p>
                </div>
                <UBadge
          ...
          <UCard class="xl:order-2 xl:col-span-2 xl:row-span-2">
            <template #header>
              <div class="flex items-start justify-between gap-3">
                ...
                <UBadge
                  :label="`${filteredDutyUsers.length} active`"
                  color="success"
                  variant="subtle"
                />
              </div>
            ...
            <div class="mb-4 overflow-x-auto pb-1">
              <UTabs
                v-model="selectedDutyBranch"
                :items="dutyBranchTabs"
                :content="false"
                size="sm"
                class="min-w-max"
              />
            ...
            <div
              v-if="filteredDutyUsers.length"
              class="max-h-[64rem] space-y-3 overflow-y-auto pr-1"
            >
              <div
                v-for="user in filteredDutyUsers"
                :key="user.logBookId"
            ...
            >
              ...
              <p class="mt-1 text-sm text-muted">
                <template v-if="selectedDutyBranch === 'all'">
                  Active users appear after time-in and remain until time-out.
                </template>
                <template v-else>
                  No active logbook session exists for this branch.
                </template>
              </p>
            </div>
          </UCard>
