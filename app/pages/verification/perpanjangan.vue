...
const memberToVerify = ref<GroupMember | null>(null);
...
// Fetch perpanjangan data
const { data, status, refresh } = await useFetch<PerpanjanganItem[]>(
  "/api/perpanjangan",
  {
    lazy: true,
    default: () => [],
...
function handleVerify(member: GroupMember) {
  memberToVerify.value = member;
  // Open verify in new tab
...
const columns: TableColumn<PerpanjanganItem>[] = [
  {
    id: "no",
    header: "NO",
    cell: ({ row }) => {
...
<template>
  <UDashboardPanel>
    <template #header>
      <UDashboardNavbar title="Verifikasi Perpanjangan">
        <template #leading>
          <UButton
