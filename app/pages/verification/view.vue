...
// Get current application doc from store
...
const ratingsList = computed(() => {
  return currentDoc.value?.appRatings?.map((r: any) => r.rating.rating) || [];
});
</script>

<template>
  <UDashboardPanel v-if="currentDoc">
    <template #header>
  ...
  <UDashboardPanel v-else>
    ...
    <template #body>
      <div class="flex flex-col items-center justify-center py-12">
        ...
        <p class="text-muted">Dokumen tidak ditemukan di store</p>
        <UButton
          class="mt-4"
          label="Kembali"
          color="primary"
