...
interface OnGoingIssue {
  ...
  finish?: string | null
  isClosed?: boolean
  equipment?: { equipment: string | null } | null
...
const frequencyStatusRows = computed(() => {
  ...
  while (i < rows.length) {
    ...
    const currentGroup = `${currentRow.dutyReportId}-${currentRow.cwp}`
    let span = 1
    while (i + span < rows.length) {
      const nextRow = rows[i + span]
      if (!nextRow || `${nextRow.dutyReportId}-${nextRow.cwp}` !== currentGroup) break
      span++
    }
    currentRow._rowspanCwp = span
    currentRow._isFirstInGroup = true
    currentRow._groupNo = groupNo
    for (let j = 1; j < span; j++) {
      const row = rows[i + j]
      if (!row) continue
      row._rowspanCwp = 0
      row._isFirstInGroup = false
    }
    i += span
    groupNo++
  }
...
function formatOnGoingIssue(issue: OnGoingIssue) {
  return issue.equipment?.equipment || issue.other || `Issue ${issue.id}`
}

function getRelatedOnGoingIssues(dutyReport: DutyReport) {
...
async function loadDailyLogbook() {
  ...
  try {
    ...
    toast.add({
      title: 'Load Failed',
      description: getRequestError(error, 'Failed to load daily logbook.'),
      color: 'error'
    })
  } finally {
    loading.value = false
...
<template>
  <UDashboardPanel id="daily-logbook-ga">
    <template #header>
      <UDashboardNavbar title="Daily Logbook (General Admin)">
        <template #leading>
    ...
    <template #body>
      <div class="space-y-4 p-4 sm:p-6">
          ...
          <template #header>
            <div class="flex flex-col gap-1">
              ...
              <p class="text-sm text-muted">
                Choose the date and branch to view daily logbook data.
              </p>
            </div>
          </template>
          ...
          <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
            ...
            <UFormField label="Branch" required>
              <USelect
                ...
                :items="branches.map((b) => ({ label: b.branch, value: b.id }))"
        ...
        >
          Choose date, branch, and click Load Data to show daily logbook.
        </div>
