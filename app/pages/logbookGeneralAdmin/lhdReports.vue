...
async function loadLhdReports() {
  ...
  try {
    ...
    toast.add({
      title: "Load Failed",
      description: getRequestError(error, "Failed to load LHD report recap."),
      color: "error",
    });
  } finally {
    loading.value = false;
...
function downloadCsv() {
  ...
  link.href = url;
  link.download = `lhd-report-recap-${safeFilePart(selectedBranch?.branch || "branch")}-${safeFilePart(startDate.value)}-to-${safeFilePart(endDate.value)}.csv`;
...
<template>
  <UDashboardPanel id="lhd-report-recap-general-admin">
    <template #header>
    ...
    <template #body>
      <div class="space-y-4 p-4 sm:p-6">
          ...
          <template #header>
            <div class="flex flex-col gap-1">
              ...
              <p class="text-sm text-muted">
                Load LHD report data by branch and occurrence date/time.
              </p>
            </div>
          </template>
        ...
        <UCard>
          ...
          >
            Select branch and date range, then click Load Data to show LHD report recap.
          </div>
