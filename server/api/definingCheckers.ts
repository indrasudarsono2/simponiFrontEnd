// Mock API endpoint for defining checkers
...
export default defineEventHandler(async (event) => {
  ...
  // Based on adminMindMap.json structure for JAKARTA - ACC with correct events
  ...
  // GET - Fetch all defining checkers
  if (method === "GET") {
    // Simulate network delay
    await new Promise((resolve) => setTimeout(resolve, 300));

    return mockDefiningCheckers;
  }

  // POST - Create new defining checker
  if (method === "POST") {
    ...
    // Create new defining checker
    const newDefiningChecker = {
      id: Math.max(...mockDefiningCheckers.map((d) => d.id)) + 1,
    ...
    // await db.definingCheckers.create(newDefiningChecker)
