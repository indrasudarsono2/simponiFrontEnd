...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Forward to backend
    const response = await $fetch(`http://${ip.ipBackEnd}/api/ielpUser`, {
      method: "POST",
    ...
    console.error("IELP creation error:", error);
    throw createError({
      statusCode: error?.statusCode || 500,
      statusMessage:
        error?.data?.statusMessage || error?.message || "Failed to create IELP",
      data: error?.data,
    });
  }
});
