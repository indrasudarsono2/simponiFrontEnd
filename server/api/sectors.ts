...
export default defineEventHandler(async (event) => {
  ...
  // GET - Fetch all sectors for ACC branch unit
  if (method === "GET") {
  ...
  // POST - Create new sector (always for ACC branch unit)
  if (method === "POST") {
    ...
    // Check for duplicate names within ACC branch unit
    const isDuplicate = mockSectors.some(
      (sector) => sector.name.toLowerCase() === body.name.toLowerCase(),
    );
    ...
    // Create new sector (always for ACC)
    const newSector = {
      id: Math.max(...mockSectors.map((s) => s.id), 0) + 1,
      name: body.name,
    ...
    // await db.sectors.create(newSector)
  ...
  // PUT/PATCH - Update sector (always for ACC branch unit)
